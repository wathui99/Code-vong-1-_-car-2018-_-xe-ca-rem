...
#define DIR10	3
#define DIR11   6
#define SERVO_CENTER		3000	//Sai số của cần sensor trên xe (am se dich qua trai, duong se dich qua phai)
#define ANGLE_MAX           150
#define STEP				5

volatile float special_ratio;
volatile float PID_ratio;
volatile uint8_t pulse_v_des=5;
volatile uint8_t pulse_v_crook_des=5;
	...
	while(1)
	{
		...
		if(get_button(BTN0)) {
			...
			while (1) // lay toc do pulse encoder mong muon
			{
				led7(pulse_v_des);
				if(get_button(BTN1)) {
					if ((pulse_v_des+1) > 30) pulse_v_des=5;
					else pulse_v_des++;
				}
				if(get_button(BTN0)) {
					if ((pulse_v_des-1) <= 0) pulse_v_des=30;
					else pulse_v_des--;
				}
			...
			while (1) // lay toc do pulse encoder mong muon
			{
				...
				if(get_button(BTN1)) {
					if ((pulse_v_crook_des+1) > 50) pulse_v_crook_des=5;
					else pulse_v_crook_des++;
				}
				if(get_button(BTN0)) {
					if ((pulse_v_crook_des-1) <= 0) pulse_v_crook_des=50;
					else pulse_v_crook_des--;
				}
				if(get_button(BTN2)) break;
			}
